...
*/
  $scope.dishes = menuFactory.getDishes().query(
    function(response) {
                    $scope.dishes = response;  //response here is the actual data
                    $scope.showMenu = true;
                },
                function(response) {
                    $scope.message = "Error: "+response.status + " " + response.statusText;
...
.controller('DishCommentController',['$scope','menuFactory',function($scope,menuFactory){
  ...
  $scope.newcomments={author:"", rating: "5", comment:"", date:""};

  $scope.submitComment=function(){
    //Step 2: This is how you record the date
    $scope.newcomments.date=new Date().toISOString();
    console.log($scope.newcomment);
    // Step 3: Push your comment into the dish's comment array
    $scope.dish.comments.push($scope.newcomments);
...
*/
   ...
   $scope.promote = menuFactory.getPromotion().get({id:0})
   .$promise.then(
                    function(response){
  ...
  //$scope.specialist=corporateFactory.getLeaders(3);
